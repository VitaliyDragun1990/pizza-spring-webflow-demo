...
public class SpecialityPizzaBuilder implements Action {

    private static final Logger LOGGER = LoggerFactory.getLogger(SpecialityPizzaBuilder.class);

    @Override
    public Event execute(RequestContext request) throws Exception {
        String type = request.getRequestParameters().get("pizzaType");

        LOGGER.debug("BUILDING A SPECIALITY PIZZA: " + type);
